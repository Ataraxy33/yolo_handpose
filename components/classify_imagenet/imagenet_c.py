...
class classify_imagenet_model(object):
    def __init__(self,
        ...
        f = open("./components/classify_imagenet/imagenet.json",  encoding='utf-8')#读取 json文件
        dict_ = json.load(f)
        f.close()
...
#
    ...
    def predict(self, img, vis = False):
        with torch.no_grad():
            ...
            print("name:",self.classify_dict[str(max_index)][1])
            print("score:",round(score_, 2))
            
            return max_index,self.classify_dict[str(max_index)][1],round(score_, 2)