...
def recognize(img,algo_img,img_reco_crop,object_recognize_model,info_dict,double_en_pts,flag_click_stable):
    ...
    if (len(double_en_pts) == 2) and (flag_click_stable == True):
        ...
        if info_dict["reco_msg"] is not None and img_reco_crop is not None: # 绘制识别区域在左下角
            h,w,_ = img.shape
            # print(h, w)
            img[(h-131):(h-1),(1):(131),:] = img_reco_crop
            # print(img_reco_crop.shape)
            cv2.rectangle(img, (1,h-131), (131,h-1), (225,66,66), 3)
            cv2.putText(img, f"result:{info_dict['reco_msg']['label_msg']}, score:{info_dict['reco_msg']['score']:.2f}", (1,h-131),cv2.FONT_HERSHEY_COMPLEX, 0.65, (255, 0, 0), 3)
            cv2.putText(img, f"result:{info_dict['reco_msg']['label_msg']}, score:{info_dict['reco_msg']['score']:.2f}", (1,h-131),cv2.FONT_HERSHEY_COMPLEX, 0.65, (0,33,255),1)
        #-----------------------------------------
        ...
        # -----------------------------------------
        # cv2.putText(img,reco_msg['label_msg']['chinese_name'],(w-131,h-131),cv2.FONT_HERSHEY_PLAIN,1,(0,255,0),2)
    else:
